function [mov,tissue] = TAMCA(DC, DT, AT, seq, params, startFrame, stopFrame, Mask)
if (isfield(params,'M'))
    M = params.M;
else
    M = 24; % default axial
...
mov = zeros(H,W,totFrames);
tissue = zeros(H,W,totFrames);
[n1, m1] = size(DC);
...
%         cnt1(:,:,midFrame+(-N/2:N/2)) = cnt1(:,:,midFrame+(-N/2:N/2)) + cntN1;
        ...
        [movN1, cntN1] =  joinPatchesMiddleMasked(patchesClean, H,W, N,P,M, OL_R,OL_AX, posMidFrame, Mask);

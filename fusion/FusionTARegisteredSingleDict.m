function [mov1,mov2,tissue1, tissue2] = FusionTARegisteredSingleDict(D1C, D2C, DT, At, seq1, seq2, params, startFrame, stopFrame, Mask)
...
% parfor t = 1:numel(THRs)
    ...
    fprintf('Found %d clutter atoms in DT out of %d\n',size(DT,2)-size(DT2,2),size(DT,2));
    nm = size(DT2,1)/2;
    DTT = (DT2(1:nm,:) + DT2(1+nm:end,:))/2;    
%     D2 = [DTT, D1C, D2C];
...
%         tissue2(:,:,frame) = tN2;
        ...
        nm = size(D,1)/2;
        noise1 = patches1 - D(1:nm,:)*G2;
        noise2 = patches2 - D((nm+1):end,:)*G2;
%         G2(~stationaryAtom,:) = 0;
        patchesClean = [DTT  zeros(size(D2C,1),size(D1C,2)), zeros(size(D1C,1),size(D2C,2))]*G2 + (noise1+noise2)/2;
%         patchesClean2 = patches2 - 

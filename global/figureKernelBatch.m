function [first_exper, last_exper, ...
    ...
    ] = figureKernelBatch(IQdata, artIQ, first_exper, last_exper, Mvalues, Nvalue, taus, OMP_ERROR, ATOM_THRs)

% addpath('ompbox/');
% General Parameters
% max_exper = last_exper-first_exper+1; % total  of experiments in this job
max_exper = last_exper;
...
% Results tables
CNR_KSVDC  = zeros(max_exper,numel(Mvalues),numel(ATOM_THRs));
CNR2_KSVDC = zeros(max_exper,numel(Mvalues),numel(ATOM_THRs));
PSNR_KSVDC = zeros(max_exper,numel(Mvalues),numel(ATOM_THRs));
MSE_KSVDC  = zeros(max_exper,numel(Mvalues),numel(ATOM_THRs));
PSNR2_KSVDC = zeros(max_exper,numel(Mvalues),numel(ATOM_THRs));
MSE2_KSVDC  = zeros(max_exper,numel(Mvalues),numel(ATOM_THRs));

CNR_SVF  = zeros(max_exper,numel(Mvalues),numel(taus));
CNR2_SVF = zeros(max_exper,numel(Mvalues),numel(taus));
PSNR_SVF = zeros(max_exper,numel(Mvalues),numel(taus));
MSE_SVF  = zeros(max_exper,numel(Mvalues),numel(taus));
PSNR2_SVF = zeros(max_exper,numel(Mvalues),numel(taus));
MSE2_SVF  = zeros(max_exper,numel(Mvalues),numel(taus));
...
for i = first_exper:last_exper
    ...
    % Create the sequence
    [FinalData, ~, RFdataClean, RFdata, NSTD] = createSequence(shift, art_shift, downsampleStep, rho, SNR, IQdata, artIQ);
    [~, ~, T] = size(FinalData);
    ...
    for p = 1:numel(Mvalues)
        ...
        CNR_KSVDC(i,p,:) = partial_res;
        CNR2_KSVDC(i,p,:) = partial2;
        PSNR_KSVDC(i,p,:) = psnr_tmpk;
        PSNR2_KSVDC(i,p,:) = psnr_tmpk2;
        MSE_KSVDC(i,p,:) = mse_tmpk;
        MSE2_KSVDC(i,p,:) = mse_tmpk2;
        toc
        ...
        toc
        CNR_SVF(i,p,:) = partial_res;
        CNR2_SVF(i,p,:) = partial2;
        PSNR_SVF(i,p,:) = psnr_tmps;
        PSNR2_SVF(i,p,:) = psnr_tmps2;
        MSE_SVF(i,p,:) = mse_tmps;
        MSE2_SVF(i,p,:) = mse_tmps2;
    end

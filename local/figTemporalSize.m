function [first_exper, last_exper, CNR_GMCA, CNR_PF, CNR_BMODE, PSNR_GMCA, PSNR_PF, PSNR_BMODE, CNR_AMCA, PSNR_AMCA, CNR_FOMCA, PSNR_FOMCA, CNR_SVF, PSNR_SVF, CNR_FIR, PSNR_FIR] = ...
    figTemporalSize(IQdata, artIQ, first_exper, last_exper, Mvalue, Nvalues, OMP_ERROR, cutoffs, taus, ATOM_THRs) 
...
% Result matrices
CNR_GMCA   = zeros(max_exper,numel(Nvalues));
PSNR_GMCA  = zeros(max_exper,numel(Nvalues));
CNR_AMCA   = zeros(max_exper,numel(Nvalues),numel(cutoffs));
PSNR_AMCA  = zeros(max_exper,numel(Nvalues),numel(cutoffs));
CNR_FOMCA  = zeros(max_exper,numel(Nvalues),numel(ATOM_THRs));
PSNR_FOMCA = zeros(max_exper,numel(Nvalues),numel(ATOM_THRs));
...
for i = first_exper:last_exper
    ...
    parfor q = 1:numel(Nvalues)
        Nvalue = Nvalues(q);
        fprintf('N = %f\n',Nvalue);
        
        fprintf('GMCA DL\n');
        locparams = params;
        locparams.N = Nvalue-1;

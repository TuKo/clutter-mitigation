function D = LearnDictionaryRegistered(seq1, seq2, params)
% Learns a dictionary from IQ data from 2 registered signals.
% SEQ1 - sequence as a 3D matrix with complex data.
% SEQ2 - sequence as a 3D matrix with complex data.
% SEQ1 and SEQ2 should have the same size.
...
if (isfield(params,'N'))
    ...
    N = min(8,size(seq1,3)); %default A-lines
end
if (isfield(params,'P'))
    P = params.P;
else
...
if (isfield(params,'DICT_SIZE'))
    ...
    DICT_SIZE = ((M+1)*(N+1)*(P+1)*2)*2; %redundancy 2 by default (2 signals)
end
if DICT_SIZE < round((N+1)*(M+1)*(P+1)*2) 
...
if (~isfield(params,'frames') || ~params.frames)
    ...
    data = [extractPatches(seq1, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT); extractPatches(seq2, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT)];
    offsetFrame = offsetFrame+7;
    data2 = [extractPatches(seq1, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT); extractPatches(seq2, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT)];
    offsetFrame = offsetFrame+11;
    data3 = [extractPatches(seq1, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT); extractPatches(seq2, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT)];
    ...
    data3 = [extractPatches(seq1, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT); extractPatches(seq2, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT)];
    offsetFrame = offsetFrame+stepFrame;
    data4 = [extractPatches(seq1, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT); extractPatches(seq2, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT)];
    offsetFrame = offsetFrame+stepFrame;
    data5 = [extractPatches(seq1, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT); extractPatches(seq2, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT)];
    offsetFrame = offsetFrame+stepFrame;
    data6 = [extractPatches(seq1, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT); extractPatches(seq2, offsetFrame, N, P, M, OL_R_DICT, OL_AX_DICT)];
...
if ~isfield(params,'Dinit') || isempty(params.Dinit)  || partition == 1.0
    params.dictsize = round(DICT_SIZE*partition);
else
    params.initdict = params.Dinit;
end

function [first_exper, last_exper, CNR_GMCA, CNR_PF, CNR_BMODE, PSNR_GMCA, PSNR_PF, PSNR_BMODE, CNR_AMCA, PSNR_AMCA, CNR_FOMCA, PSNR_FOMCA, CNR_SVF, PSNR_SVF, CNR_FIR, PSNR_FIR, CNR_GMCA2, PSNR_GMCA2] = ...
     figCorrelation(IQdata, artIQ, first_exper, last_exper, rhos, Mvalue, Nvalue, OMP_ERROR, cutoffs, taus, ATOM_THRs) 
...
% Result matrices
CNR_GMCA   = zeros(max_exper,numel(rhos));
PSNR_GMCA  = zeros(max_exper,numel(rhos));
CNR_GMCA2  = zeros(max_exper,numel(rhos));
PSNR_GMCA2 = zeros(max_exper,numel(rhos));
CNR_AMCA   = zeros(max_exper,numel(rhos),numel(cutoffs));
PSNR_AMCA  = zeros(max_exper,numel(rhos),numel(cutoffs));
CNR_FOMCA  = zeros(max_exper,numel(rhos),numel(ATOM_THRs));
PSNR_FOMCA = zeros(max_exper,numel(rhos),numel(ATOM_THRs));
...
for i = first_exper:last_exper
    ...
    parfor q = 1:numel(rhos)
        fprintf('Rho = %f\n',rhos(q));
        rh = rhos(q);
        [FinalData, Noise, RFdataClean, ~, NSTD] = createSequence(shift, art_shift, downsampleStep, rh, SNR, IQdata, artIQ);
        [~, ~, T] = size(FinalData);
        ...
        [~, ~, RFDataSampleForTissue, ~, ~] = createSequence(shift, art_shift, downsampleStep, rh, SNR, IQdata, artIQ);
...
% save D:\javier_temp\resultsCorrelation.mat

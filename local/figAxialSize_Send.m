...
for i = 1:procs
    ...
    j{i} = batch(c,'figAxialSize',nargout(@figAxialSize),{IQdata, artIQ, first, last, Mvalues, Nvalue, OMP_ERROR, cutoffs, taus, ATOM_THRs},'Pool',poolprocs,'AttachedFiles','ompbox/');
end
...
save('results\AxialSize-jobs.mat','jobsID','max_exper','procs','Mvalues','Nvalue','OMP_ERROR','cutoffs', 'taus', 'ATOM_THRs');
diary(j{1});
...
clear
load('results\AxialSize-jobs.mat');

nvalues = numel(Mvalues);
...
%save the results
clear cluster job r 
save('results\axialsize.mat');

%% plot figures:
clear
load('results\axialsize.mat');
folder = 'results';
close all
fontsz = 20;
...
%PSNR
figure;
plot(Mvalues, mean_UN*ones(numel(Mvalues),1),':k','linewidth',2,'markersize',15);
hold on;
plot(Mvalues, mean_PF*ones(numel(Mvalues),1),'--k','linewidth',2,'markersize',15);

errorbar(Mvalues, mean_GMCA, std(PSNR_GMCA), 's-r','linewidth',2,'markersize',15);

[PSNR, IDX1] = max(mean_AMCA,[],2);
STD1 = squeeze(std(PSNR_AMCA));
STD1 = STD1(sub2ind(size(STD1),ones(size(IDX1)),IDX1));
errorbar(Mvalues, PSNR, STD1, 'v-b','linewidth',2,'markersize',15);

[PSNR, IDX1] = max(mean_FOMCA,[],2);
STD1 = squeeze(std(PSNR_FOMCA));
STD1 = STD1(sub2ind(size(STD1),ones(size(IDX1)),IDX1));
errorbar(Mvalues, PSNR, STD1, 'o-g','linewidth',2,'markersize',15);

[PSNR, IDX1] = max(mean_SVF,[],2);
STD1 = squeeze(std(PSNR_SVF));
STD1 = STD1(sub2ind(size(STD1),ones(size(IDX1)),IDX1));
errorbar(Mvalues, PSNR, STD1,'x-c','linewidth',2,'markersize',15);

xlim([5 100]);
ylim([30 55]);
xlabel('Axial Size (Elements)','FontSize',fontsz);
...
print([folder '\figureAxialSize.eps'],'-depsc2','-r600');
print([folder '\figureAxialSize.png'],'-dpng');
